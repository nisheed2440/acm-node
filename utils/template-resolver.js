...
async function templateResolver(req, res, next) {
    ...
    // Get the template URL from AEM API
    const urlObj = await getData(`http://localhost:9000/template/api/${requestURL}`);
    ...
    if (urlObj.err) {
        ...
        let data;
        try {
            data = JSON.parse(urlObj.body);
        } catch (err) {
            next(err);
            return;
        }
        // Get the template from template cache on NGINX
        const templateObj = await getData(data.url);
        
        if (templateObj.err) {
            next(templateObj.err);
        } else {
            const outputTemplate = await componentResolver(templateObj.body);
